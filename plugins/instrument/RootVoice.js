// RootVoice — plays only the root note of the active key.
// The "original" sound: minimal, pitch-only, no harmonic colouring.
...
export class RootVoice {
  static type = 'instrument'
  static id = 'root-voice'
  static version = '1.0.0'
  static label = 'Root Note'
  static description = 'Single root note — pitch only, no chord'
  ...
  onKeyPress(code) {
    ...
    osc.frequency.value = noteFreq(key.root, 4)
    gain.gain.setValueAtTime(0, now)
    gain.gain.linearRampToValueAtTime(this.volume, now + this.attack)

    osc.connect(gain)
    gain.connect(ctx.destination)
    osc.start(now)

    this._nodes.set(code, [{ osc, gain }])
  }
  ...
  _stopAll() {
    for (const code of [...this._nodes.keys()]) this.onKeyRelease(code)
  }
